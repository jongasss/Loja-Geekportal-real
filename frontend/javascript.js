...
if (login2) {
  if (localStorage.getItem("usuario") !== null) {
    location.pathname = "frontend/usuario.html";
  }
}
...
async function criarProduto(produto) {
  ...
  if (localStorage.getItem("usuario") !== null) {
    ...
    const response = await fetch(
      `http://localhost:5000/produto/favorito?usuario=${usuario.id}&produto=${produto.id}`
    );
    ...
    botaoFavoritos.onclick = async function () {
      if (botaoFavoritos.classList.contains("adicionado-ao-carrinho")) {
        const response = await fetch(
          `http://localhost:5000/produto/favorito/remover`,
          {
            method: "DELETE",
            headers: {
        ...
        const response = await fetch(
          `http://localhost:5000/produto/favoritar`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
async function login(event) {
  ...
  const response = await fetch(
    `http://localhost:5000/usuario/login?email=${email}&senha=${senha}`
  );
...
async function cadastrarProduto(event) {
  ...
  const response = await fetch("http://localhost:5000/produto/cadastrar", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function editarProduto(event) {
  ...
  const response = await fetch(
    "http://localhost:5000/produto/atualizar/" + id,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
async function deletarProduto(event) {
  ...
  const response = await fetch("http://localhost:5000/produto/deletar/" + id, {
    method: "DELETE",
  });
...
async function pegarFavoritos() {
  ...
  const response = await fetch("http://localhost:5000/produto/favoritos/" + id);

  const results = await response.json();

  for (let produto of results.data) {
    criarProdutoFavoritosUsuario(produto);
  }
}
...
async function alterarConta(event) {
  ...
  const response = await fetch("http://localhost:5000/usuario/editar/" + id, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (results.success) {
    location.pathname = "frontend/index.html";
    localStorage.setItem("usuario", JSON.stringify({ ...data, id, cargo }));
  } else {
    alert(results.message);
  }
}
